"""Добавление валидации вводимых пользователем данных"""
...
class Printer(OrgTech):
    ...
    def __init__(self, name, price, qty):       # конструктор принтеров
        ...
        if type(price) != int or type(qty) != int:
            raise TypeError('Проверьте введенные значения цены и кол-ва у Принтеров. '
                            'Они должны являться числами.')
        if qty < 0:
            raise ValueError('Проверьте введенные значения цены у Принтеров. '
    ...
    def post_product(self):
        count = 0                                       # счетчик списаний товара
        for row in Store.data:                        # экспорт принтеров и списание с базы оргтехники
            if row['name'] == self.d['name']:           # и из списка принтеров
                row['qty'] -= self.d['qty']
                count += 1
        if count == 0:                              # если товар не списался, значит его не было в наличии
            raise ValueError(f'Товара {self.name} нет на складе. Выберите другую модель.')
...
class Scanner(OrgTech):
    ...
    def __init__(self, name, price, qty):
        ...
        if type(price) != int or type(qty) != int:
            raise TypeError('Проверьте введенные значения цены и кол-ва у Сканеров. '
                            'Они должны являться числами.')
        if qty < 0:
            raise ValueError('Проверьте введенные значения цены у Сканеров. '
                             'Они должны быть положительными.')
    ...
    def post_product(self):
        count = 0
        for row in Store.data:                        # экспорт сконеров и списание с базы оргтехники
...
class Xerox(OrgTech):
    ...
    def __init__(self, name, price, qty):
        ...
        self.d['type'] = self.type
        if type(price) != int or type(qty) != int:
            raise TypeError('Проверьте введенные значения цены и кол-ва у Ксероксов. '
                            'Они должны являться числами.')
        if qty < 0:
            raise ValueError('Проверьте введенные значения цены у Ксероксов. '
                             'Они должны быть положительными.')
